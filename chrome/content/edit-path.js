(function () {
    var SurfKeysPathEditor = {
        ...
        */
        getUrl: function () {
            var currloc = window.arguments[0],
                url = currloc.loc.replace(/(^\w+:\/{2,3}|\?.*$)/g, '').split('/');
            return {
        ...
        */
        init: function (event) {
            var scope = {}, values, inputs, customPath;
            Components.utils.import("resource://surfkeysmodules/log.jsm", scope);
            values = SurfKeysPathEditor.getUrl();
            // get input elements
            inputs = {
    ...
    window.addEventListener('load', SurfKeysPathEditor.init, false);
    window.addEventListener('dialogaccept', SurfKeysPathEditor.save, false);
    window.addEventListener('dialogcancel', SurfKeysPathEditor.cancel, false);
}());
